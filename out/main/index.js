import { ipcMain, clipboard, app, shell, BrowserWindow } from "electron";
...
function buildRecordPath(records, record, guard = 0) {
  ...
  if (parentRef === void 0 || parentRef === null || parentRef === record.record_number || parentRef === 5) {
    return name;
  }
...
function folderContainsType(records, tree, folderRef, typeFilter, memo = /* @__PURE__ */ new Map()) {
  if (!typeFilter) return true;
...
function getDriveHighlights(drive, options = {}) {
  ...
  const folders = summary.slice().sort((left, right) => (right.size_bytes ?? 0) - (left.size_bytes ?? 0)).slice(0, limit).map((folder) => ({
  ...
  const files = Object.values(records).filter((record) => record && record.is_dir === false).sort((left, right) => (right.size_bytes ?? 0) - (left.size_bytes ?? 0)).slice(0, limit).map((record) => ({
...
function getDriveDistribution(drive, options = {}) {
  ...
  const summary = Array.isArray(driveCache.summary) ? driveCache.summary : [];
  const records = driveCache.records ?? {};
  const rootFolders = summary.slice().sort((left, right) => (right.size_bytes ?? 0) - (left.size_bytes ?? 0)).slice(0, limit).map((folder) => ({
  ...
  const fileTypes = [...typeBuckets.values()].sort((left, right) => right.size_bytes - left.size_bytes).slice(0, limit).map((entry) => ({
    ...entry,
...
async function readPathPreview(targetPath) {
  ...
  if (kind === "text") {
    const buffer = await readFile(targetPath);
    const content = buffer.toString("utf8").slice(0, 12e4);
    return {
      kind,
      path: targetPath,
      name: targetPath.split("\\").pop() ?? targetPath,
      content,
      truncated: buffer.length > 12e4
    };
  }
  return {
    kind: "unsupported",
